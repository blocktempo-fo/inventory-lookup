(function () {
  ...
  // ★ 圖片政策：只放「100% 確認對應到實體型號」的圖
  // 規則：（1）型號明確 +（2）官網/品牌原圖 +（3）無誤導風險
  // 不確定就不放 → 自動 fallback 到類別 SVG 圖示
  const PRODUCT_IMAGES = [
    // ─── DJI 官方產品（型號可從 DJI URL 確認）───
    ...
    ['FW50原廠電池充電器', 'https://www.adorama.com/images/product/isobctrw.jpg'],      // BC-TRW
    ...
    // ⚠️ 移除：Rode 指向性麥克風（不確定是否 VM-NTG）
    // ⚠️ 移除：夾式麥克風(品牌未知)、ZOOM H7(原圖是 H6)、Wireless PRO、VideoMicro
    ...
    // ⚠️ 移除所有其他腳架/燈架/兔籠/輪座/看板：品牌型號不確定
      ...
      dom.locationMap.innerHTML = `
        ...
        <div class="location-map-container">
          <div class="location-map-inner">
            <img src="./location-map.svg?v=8" alt="器材室收納位置圖" class="location-map" />
            ${overlays}
          </div>
        </div>
        <div class="loc-list">
      ...
      if (!overlay) {
        ...
        overlay.innerHTML = `
          ...
          <div class="map-zoom-scroll">
            <img src="./location-map.svg?v=8" alt="器材室收納位置圖（放大）" class="map-zoom-img" />
          </div>`;
  ...
  function setupHowto() {
    ...
    const okBtn = document.querySelector('#howtoOk');
    const closeBtn = document.querySelector('#howtoClose');
    const triggerBtn = document.querySelector('#howToBtn');
    if (!overlay) return;
